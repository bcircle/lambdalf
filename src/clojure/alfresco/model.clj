...
(defn ^NamespaceService namespace-service
  []
  (.getNamespaceService (c/alfresco-services)))

(defn ^DictionaryService dictionary-service
  []
...
(defmethod qname :identity
  [^QName q]
  q)

(defmethod qname :create
  [qname-expr]
  (if qname-expr (QName/createQName qname-expr)))

(defmethod qname :str-shortname
  [^String qname-str]
...
(defn ^String namespace-uri
  "Returns the namespace URI of a QName."
  [^QName q]
  (.getNamespaceURI q))

(defn ^String local-name
...
(defn in?
  ...
  [ns ^QName qname]
  (let [qname-ns (namespace-uri qname)]
    (or
      (= qname-ns ns)
...
(defn ^String valid-local-name
  "Returns a valid QName 'local name' for any given string."
  [s]
  (QName/createValidLocalName s))
