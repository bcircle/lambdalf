...
(defn ^ContentService content-service
  []
...
(defmethod write! InputStream
  ([^InputStream src node]          (.putContent (get-writer node) src))
  ([^InputStream src node property] (.putContent (get-writer node property) src)))

(defmethod write! String
  ([^String src node]          (write! (ByteArrayInputStream. (.getBytes src "UTF-8")) node))
  ([^String src node property] (write! (ByteArrayInputStream. (.getBytes src "UTF-8")) node property)))

(defmethod write! File
  ([^File src node]          (.putContent (get-writer node) src))
  ([^File src node property] (.putContent (get-writer node property) src)))
